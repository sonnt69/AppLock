...
public class PatternLockView extends View {
    ...
    private Bitmap bitmapPointBitmapSrc;
    private Bitmap bitmapPointBitmapOnSrc;
    private Bitmap bitmapPointBitmapOnSrcError;
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            nodeSrc = typedArray.getDrawable(R.styleable.PatternLockView_Lock9View_lock9_nodeSrc);
            nodeOnSrc = typedArray.getDrawable(R.styleable.PatternLockView_Lock9View_lock9_nodeOnSrc);
            nodeOnSrcError = typedArray.getDrawable(R.styleable.PatternLockView_Lock9View_lock9_nodeOnSrc_error);

            bitmapPointBitmapSrc = ((BitmapDrawable) nodeSrc).getBitmap();
            bitmapPointBitmapOnSrc = ((BitmapDrawable) nodeOnSrc).getBitmap();
            bitmapPointBitmapOnSrcError = ((BitmapDrawable) nodeOnSrcError).getBitmap();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        List<Float> centerXList = new ArrayList<>();
...
//
//                for (int a = 0; a < sDotCount; a++) {
//                    for (int b = 0; b < sDotCount; b++) {
//                        DotState dotState = mDotStates[a][b];
//                        float translationY = dotState.mTranslateY;
//                        drawCircleOnSrc(canvas, (int) centerX, (int) centerY + translationY, drawLookupTable[a][b], dotState.mAlpha);
//                    }
//                }

                DotState dotState = mDotStates[0][0];
                drawCircleOnSrc(canvas, (int) centerX, (int) centerY, drawLookupTable[0][0], dotState.mAlpha);
            }
...
//             Draw last in progress section
            ...
            for (int i = 0; i < centerXList.size(); i++) {
                DotState dotState = mDotStates[0][0];
                float translationY = dotState.mTranslateY;
                drawCircleOnSrc(canvas, (int) centerXList.get(i).intValue(), (int) centerYList.get(i).intValue() + translationY, drawLookupTable[0][0], dotState.mAlpha);
            }
        }
    }
...
//        canvas.drawCircle(centerX, centerY, size / 2, mDotPaint);
        canvas.drawBitmap(bitmapPointBitmapSrc, centerX - bitmapPointBitmapSrc.getWidth() / 2, centerY - bitmapPointBitmapSrc.getHeight() / 2, mDotPaint);
    }


    private void drawCircleOnSrc(Canvas canvas, float centerX, float centerY,
                                 boolean partOfPattern, float alpha) {
//        mDotPaint.setColor(getCurrentColor(partOfPattern));
//        mDotPaint.setAlpha((int) (alpha * 255));
//        canvas.drawCircle(centerX, centerY, size / 2, mDotPaint);
        if (wrongPass) {
            canvas.drawBitmap(bitmapPointBitmapOnSrcError, centerX - bitmapPointBitmapOnSrcError.getWidth() / 2, centerY - bitmapPointBitmapOnSrcError.getHeight() / 2, mDotPaint);
        } else {
            canvas.drawBitmap(bitmapPointBitmapOnSrc, centerX - bitmapPointBitmapOnSrc.getWidth() / 2, centerY - bitmapPointBitmapOnSrc.getHeight() / 2, mDotPaint);
        }
    }
